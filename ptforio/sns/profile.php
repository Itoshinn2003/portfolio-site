...
<main>
    ...
    $sql=$pdo->prepare('select * from profile where iwitteruser_userid = ?');
    ...
    if(isset($_REQUEST['username'])){
        $sql=$pdo->prepare('update profile set  message=? where iwitteruser_userid =?');
        $sql->execute([$_REQUEST['message'],$_SESSION['user']['userid']]);
            
        
        $sql=$pdo->prepare('update iwitteruser set username=? where userid =?');
        $sql->execute([$_REQUEST['username'],$_SESSION['user']['userid']]);
    ...
    <div class="container p-5  bg-white rounded">
        ...
        <div class="row">
            <div class="col-md-7  ">
                ...
                <a href="profile-edit.php?id=<?php echo $row['userid'] ; ?> ">編集</a>
                </div>
